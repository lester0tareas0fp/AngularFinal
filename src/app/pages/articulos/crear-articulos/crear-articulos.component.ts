...
export class CrearArticulosComponent implements OnInit{
  ...
  constructor(private fb: FormBuilder, private service: ArticulosService) 
  { 
  ...
  ngOnInit()
  {
    this.crearArticuloForm = this.fb.group(
      {
        'imagen': [''],
        'articulo': ['', [Validators.required, Validators.minLength(2)]],
        'descripcion': ['', [Validators.required, Validators.minLength(2)]],
        'fabricante': ['', [Validators.required, Validators.minLength(2)]],
        ...
        'stock1': [0, [Validators.min(0), RxwebValidators.numeric({allowDecimal:false})]],
        'stock2': [0, [Validators.min(0), RxwebValidators.numeric({allowDecimal:false})]],
        'stock3': [0, [Validators.min(0),  RxwebValidators.numeric({allowDecimal:false})]],
      }
    )
  }
  ...
  crearArticulo()
  {
    ...
    if(this.crearArticuloForm.invalid)
    {
      return;
    }
    ...
    {
      'articulo': this.crearArticuloForm.get('articulo')?.value,
      'descripcion': this.crearArticuloForm.get('descripcion')?.value,
      'fabricante': this.crearArticuloForm.get('fabricante')?.value,
      'peso': this.crearArticuloForm.get('peso')?.value,
      'largo': this.crearArticuloForm.get('largo')?.value,
      'ancho': this.crearArticuloForm.get('ancho')?.value,
      'alto': this.crearArticuloForm.get('alto')?.value,
      'precio': this.crearArticuloForm.get('precio')?.value,
      'nombre_imagen': this.imagen.nombrE_IMAGEN,
      'imagen': this.imagen.imagen,
      'formato': this.imagen.formato,
    }
    
    this.service.crearArticulo(articulo).subscribe( resp =>
      {
        for(let i = 1; i <= 3; i++)
        {
          const nuevo_stock: InsertarStock =
            {
              iD_ALMACEN: i,
              iD_ARTICULO: resp.id_articulo,
              cantidad: this.crearArticuloForm.get('stock'+i)?.value
            };
